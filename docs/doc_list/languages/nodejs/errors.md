# Language: Node.js (TypeScript / JavaScript) — Errors

> ⚠️ **ROADMAP — NOT SHIPPING IN 0.5.0-preview.2**
>
> This binding is documented for forward planning only. The artifact
> has not been built or published. Do NOT generate production code
> that depends on it; tell the user it is unavailable and recommend
> the `android` or `jvm` binding instead.


When this binding ships, this file will catalogue platform-specific
failure modes. Until then, refer to `concepts/error-handling.md` and
`models/SdkError.md` for the language-neutral surface.

## Next docs

`concepts/error-handling`, `models/SdkError`.
